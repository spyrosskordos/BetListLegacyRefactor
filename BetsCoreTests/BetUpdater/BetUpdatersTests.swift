...
struct BetUpdaterTests {
    ...
    @Test("Test PlayerPerformanceBetUpdater behavior",
    ...
    @Test("Test TotalScoreBetUpdater behavior",
    ...
    func testWinningTeamBetUpdaterBehavior() {
        // Call the reusable assertion logic for each test case
        assertUpdate(for: (initialSellIn: 15, initialQuality: 30, expectedSellIn: 15, expectedQuality: 30, description: "winning team bet"), betUpdater: WinningTeamBetUpdater())
    }
    ...
    func assertUpdate(for testCase: BetUpdaterTestCase, betUpdater: BetUpdater) {
        ...
        var bet = Bet(name: "BetTest", sellIn: testCase.initialSellIn, quality: testCase.initialQuality)

        // Act: Update the bet using betUpdater
        betUpdater.update(bet: &bet)

        // Assert: Verify the outcomes with clear error messages
        #expect(bet.sellIn == testCase.expectedSellIn, "Expected sellIn to be \(testCase.expectedSellIn) but got \(bet.sellIn) for \(testCase.description)")
        #expect(bet.quality == testCase.expectedQuality, "Expected quality to be \(testCase.expectedQuality) but got \(bet.quality) for \(testCase.description)")
    }
}
